export async function POST(req) {
  ...
  const SEARCH_PROMPTS = [
    ...
    "Find 3 ancient-craft-meets-contemporary-design fashion brands using traditional techniques from indigenous or artisan communities, elevated to luxury. The kind worn by style icons who know things others dont. For each: name, sub, city, country (2-letter code), continent (Africa/Americas/Asia/Europe/Oceania), cat (Knitwear/Drape/Tailoring/Denim/Workwear/Minimal/Accs), rarity (8.5-10), g (guide quote max 12 words), story (2 sentences with founder name), unsplash_query. JSON array only.",
    "Find 3 underground visual artists — painters, ceramicists, photographers, or textile artists — whose work is quietly collected by celebrities, fashion designers, or cultural icons but who have almost no public profile. Think the kind of artist whose piece hangs in a famous person home but has no Wikipedia page. For each provide: name (artist or studio name), sub (medium descriptor e.g. Earth Pigment Painting or Celadon Ceramics), city, country (2-letter code), continent (Africa/Americas/Asia/Europe/Oceania), cat (one of: Painting/Ceramic/Photo/Textile Art), rarity (8.5-10), g (guide note max 12 words, poetic), story (2 sentences with artist name and what makes them extraordinary), unsplash_query (3-4 words for an art/painting/ceramic photo). JSON array only.",
  ];
  ...
  const res = await fetch("https://api.anthropic.com/v1/messages", {
    ...
    body: JSON.stringify({
      ...
      max_tokens: 1500,
      system: "You are a cultural intelligence agent specializing in discovering ultra-niche underground fashion brands and artists collected by celebrities. Always respond with valid JSON arrays only — no markdown, no preamble, no code fences.",
      messages: [{ role: "user", content: SEARCH_PROMPTS[promptIndex] || SEARCH_PROMPTS[0] }],
    }),
  });
